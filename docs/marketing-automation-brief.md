...
## Locked decisions
- **Blog gating:** human-approved, as originally decided — but **no API key**. The post is written by a scheduled **Claude Code routine** (not a GitHub Action calling the API) following `docs/weekly-blog-post.md`. Each run opens a PR (GitHub's own notification is the email); merging is what publishes, via Cloudflare Pages. The routine needs no secrets at all. GitHub Actions only runs the deterministic, secret-free `npm run validate:blog`.
...
6. **Publisher** — insert into the generated-post data files, run `npm run validate:blog` and `npm run build`, then **open a PR**. Opening it is what notifies the owner.
